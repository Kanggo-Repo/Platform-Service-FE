...
test(
    'login route redirects directly to keycloak authorize endpoint',
    function () {
...
test(
    'callback exchanges authorization code and stores tokens in session',
    function () {
        Http::fake([
        ...
        expect(
            User::query()
                ->where('auth_subject', 'keycloak:kc-user-1')
                ->exists(),
        )->toBeTrue();
        expect(
            User::query()
                ->where('auth_subject', 'keycloak:kc-user-1')
                ->firstOrFail()->role_snapshot,
        )->toBe(['super_admin']);
        expect(
            User::query()
                ->where('auth_subject', 'keycloak:kc-user-1')
                ->firstOrFail()->permission_snapshot,
        )->toBe(['roles.manage', 'users.manage']);
    },
);

test(
    'callback returns user to remembered page after standby re-login',
    function () {
        Http::fake([
            'https://auth.example.test/realms/kanggo/protocol/openid-connect/token' => Http::response(
                [
...
test('legacy consume callback path is still accepted', function () {
    Http::fake([
        'https://auth.example.test/realms/kanggo/protocol/openid-connect/token' => Http::response(
            [
                'access_token' => 'access-token-123',
        ...
        'http://127.0.0.1:8011/api/v1/me' => Http::response([
            'data' => [
                'identity' => [
                    'subject' => 'kc-user-legacy',
                    'email' => 'legacy@example.test',
                    'name' => 'Legacy Platform User',
                ],
                'roles' => ['super_admin'],
...
test(
    ...
    function () {
        ...
        expect($response->headers->get('Location'))->toContain(
            'https://auth.example.test/realms/kanggo/protocol/openid-connect/logout',
        );
    },
);

test(
    'platform auth middleware logs out local user when shared subject cookie points to another account',
    function () {
