...
@php
    ...
    $materialCount = (int) ($summary['total_users'] ?? 0);
    $unitCount = (int) ($summary['role_count'] ?? 0);
    $storeCount = (int) ($summary['permission_count'] ?? 0);
    $workItemCount = (int) ($summary['pending_access_count'] ?? 0);
    $chartData = [
        'labels' => $dashboard['chart']['labels'] ?? [],
        'data' => $dashboard['chart']['data'] ?? [],
    ];
    $recentActivities = collect($dashboard['recent_activities'] ?? [])
        ->map(function (array $activity): object {
            return (object) [
                'name' => $activity['name'] ?? '-',
                'category' => $activity['category'] ?? 'Menunggu Akses',
                'category_color' => $activity['category_color'] ?? 'primary',
                'created_at_human' => $activity['updated_at_human'] ?? '-',
            ];
        });
@endphp
...
<div class="stats-grid-container mb-3">
    ...
    <div class="modern-stat-card">
        ...
        <div class="card-content">
            ...
            <h2 class="card-value text-shadow-bottom">@format($materialCount)</h2>
        </div>
    ...
    <div class="modern-stat-card">
        ...
        <div class="card-content">
            ...
            <h2 class="card-value text-shadow-bottom">@format($unitCount)</h2>
        </div>
    ...
    <div class="modern-stat-card">
        ...
        <div class="card-content">
            ...
            <h2 class="card-value text-shadow-bottom">@format($workItemCount)</h2>
        </div>
    ...
    <div class="col-lg-5">
        ...
        <div class="card border-0 shadow-sm rounded-4">
                ...
                <div class="list-group list-group-flush py-1">
                    @forelse($recentActivities as $activity)
                        <div class="list-group-item border-0 px-3 py-1.5 d-flex align-items-center hover-bg-light transition-base">
                            ...
                            <small class="ms-2 whitespace-nowrap text-muted" style="font-size: 0.7rem;">{{ $activity->created_at_human }}</small>
                        </div>
                    @empty
            ...
            <div class="card-footer bg-white border-0 px-3 pb-3 pt-0">
                <a href="{{ route('settings.users.index') }}" class="btn btn-light btn-sm w-100 fw-medium text-shadow-bottom rounded-3" style="font-size: 0.8rem; color: #000000 !important">Lihat Semua</a>
            </div>
            @endif
        </div>
    </div>
